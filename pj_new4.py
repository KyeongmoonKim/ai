...
for line in train_reader:
	...
	if(num%1000 == 0 and num > 0):
		...
		for test_line in test_reader:
			...
			if(len(test_sub_list)<4): #one or zero point.
				...
				continue
			while len(test_sub_list) < 30: #so small points
				test_sub_list = add_mid_point(test_sub_list)
			test_sub_list = vectorize_term(test_sub_list, 15) #compress to 9 vectors
			test_sub_list = normalize(test_sub_list) #normalize
			if(int(len(test_sub_list)/2) != 15):
				print("the length of vector isn't 15")
				not_passed = not_passed+1
				continue
			...
			if(int(len(test_sub_list)/2)!=14):
				print("not 14")
			test_X = np.array(test_sub_list).reshape(14, 2)
			Y = []
	...
	if(len(sub_list)<4): #one or zero point.
		not_passed = not_passed+1
		continue
	while len(sub_list) < 30: #so small points, mid point insertion
		sub_list = add_mid_point(sub_list)
	sub_list = vectorize_term(sub_list, 15) #compress to 9 vectors
	sub_list = normalize(sub_list)
	if(int(len(sub_list)/2) != 15):
		print("the length of vector isn't 15")
		not_passed = not_passed+1
		continue
	sub_list = data_process2(sub_list) #compute the angle difference vector.
	sub_list = random_shake(sub_list) #insert noise.
	if(int(len(sub_list)/2)!=14):
		print("not 8")
	X = np.array(sub_list).reshape(14, 2)
